...
* Which of these statements will compute the number of cases that are outliers with respect to *height*?  (Remember that the data frame is named `Galton`.)
```{r include=FALSE}
...
```
* How many of the cases are outliers in *height* ? `r I(outliers)`
...
```
* How many of the cases are outliers in *mother*? `r I(outliers2)`
...
```
* How many of the cases are outliers in *father*? `r I(outliers3)`
