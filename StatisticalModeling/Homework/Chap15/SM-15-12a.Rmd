...
I(textItem(name="Explain", totalPts=2, rows=3))
```
...
`r I(f(FALSE))` It's a good idea.    
`r I(f(FALSE))` Bad idea since a person's sex probably has nothing to do with his or her score.        
`r I(f(TRUE))` Useless, since *sex* is redundant with *student*.    
...
In short, the interaction term adds a lot of new degrees of freedom --- as many as in the \VN{student} variable itself --- and reduces the degrees of freedom in the residual accordingly.  
</aside>
