...
I(textItem(name="weight", totalPts=2, rows=4))
```
...
```{r include=FALSE, eval=FALSE}
I(textItem(name="income", totalPts=2, rows=4))
```
...
```{r include=FALSE, eval=FALSE}
I(textItem(name="speed", totalPts=2, rows=4))
```
...
* Systolic blood pressure in adults.  [You might need to look this
  up on the Internet.]
```{r include=FALSE, eval=FALSE}
I(textItem(name="bp", totalPts=2, rows=4))
```
...
* Blood cholesterol LDL levels. [Again, you might need the
  Internet.]
```{r include=FALSE, eval=FALSE}
I(textItem(name="cholesterol", totalPts=2, rows=4))
```
...
* Fuel economy among different models of
  cars.
```{r include=FALSE, eval=FALSE}
I(textItem(name="car", totalPts=2, rows=4))
```
...
```{r include=FALSE, eval=FALSE}
I(textItem(name="wind", totalPts=2, rows=4))
```
...
```{r include=FALSE, eval=FALSE}
I(textItem(name="sleep", totalPts=2, rows=4))
```
