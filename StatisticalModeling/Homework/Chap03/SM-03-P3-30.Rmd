...
*SNF* variable records the amount of nitrogen fixed in soil by
bacteria that reside in root nodules of the plants.  Make a box plot
and a histogram and describe the distribution.     
...
<code>log</code>, <code>log2</code>, or <code>log10</code>.  Which of these functions
was used to compute the quantity *logSNF* from *SNF*. (Hint: Try
them out!)
```{r include=FALSE}
...
scale.  For example, <code>log2</code> calculates the base-2 logarithm.  On
